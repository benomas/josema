<div>
	<form id="informacionProducto" name="informacionProducto">	
		...
		?>
		<div  class="contenedor_imagen_middle toottip_class_dialog <?php if( !empty($producto->promocion) ) {?> promocion_container<?php }?>" onclick="mostrarOriginal('<?php echo $backgoundImage;?>');" title="Maximizar imagen">
		...
		<div class="contenedor_info toottip_class_dialog" <?php if(!empty($producto->promocion)){?> title="<?php echo title_promocion($producto->promocion);?> <?php }?>">
			<table class="table table-striped table-bordered table-hover table_expanded">
				...
				<tr >
					...
					<td class="">$<label class="concepto_value precio_producto" id="producto_precio_div_<?php echo $producto->id_inventario;?>" ><?php echo round($producto->precio,2); ?></label>
					</td>
				</tr>
				<?php 
				}
		...
		<div class="bno-accions acciones">
			<div class="icon- bno-button tooltip_class_accion" title="Reducir" onclick="reducir('<?php echo $producto->id_inventario; ?>');">
			&#xe683
			</div>
			<?php 
...
$(document).ready(function()
{
	...
	{
	?>	
		calcularPrecioDiv(JSON.parse('<?php echo json_encode($producto);?>'));
	<?php 
	}
	?>	
});

function calcularPrecioDiv(producto)
{	
	if(producto!='')
	{
		switch(producto.promocion.nombre_tabla_promocion)
		{
			case 'ci_promocion_fija':
										$('#producto_precio_div_' + producto.id_inventario).text(producto.promocion.precio_oferta);
										break;
		}
	}
}
